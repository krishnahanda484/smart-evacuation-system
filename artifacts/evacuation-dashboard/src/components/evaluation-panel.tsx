...
import { BarChart2, Play, Loader2, Scale, GitBranch } from 'lucide-react';
...
export function EvaluationPanel() {
  ...
  const handleRun = () => {
    runMut.mutate({
      data: { layout_id: layoutId, population, adherence_rate: adherence, n_runs: nRuns },
    });
  };

  // Congestion time series chart (downsampled)
  ...
  const rerouting  = results?.rerouting;
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MetricCard
                ...
                value={`${imp > 0 ? '+' : ''}${imp.toFixed(1)}%`}
                sub="ML-guided vs greedy"
                good={imp > 0}
              />
              <MetricCard
                label="Greedy Mean Time"
                value={`${greedyMean.toFixed(1)}s`}
                sub={`±${greedyStd.toFixed(1)}s`}
              />
              <MetricCard
                label="Guided Mean Time"
                value={`${guidedMean.toFixed(1)}s`}
                sub={`±${guidedStd.toFixed(1)}s`}
                good={guidedMean < greedyMean}
              />
              <MetricCard
                label="Wall Time"
                value={`${wallTime.toFixed(0)}s`}
                sub={`${nRuns * 2} total runs`}
              />
            </div>
            ...
            {/* ── Rerouting mechanism panel ──────────────────────────── */}
            {rerouting && (
              <div className="bg-card border border-amber-500/30 rounded p-4 flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  <GitBranch className="w-4 h-4 text-amber-400" />
                  <h3 className="text-xs font-bold text-foreground">Rerouting Mechanism Analysis</h3>
                  <span className="ml-auto text-[9px] font-mono text-amber-400/80 bg-amber-900/40 border border-amber-500/30 px-1.5 py-0.5 rounded">
                    Paper §IV-C
                  </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <div className="bg-background border border-border rounded p-2 text-center">
                    <p className="text-[9px] text-muted-foreground uppercase tracking-wider font-semibold">Rerouted Zones</p>
                    <p className="text-lg font-mono font-bold text-amber-400 mt-0.5">
                      {rerouting.total_reroute_decisions?.toLocaleString() ?? '—'}
                    </p>
                    <p className="text-[9px] text-muted-foreground font-mono">
                      of {rerouting.total_zone_decisions?.toLocaleString() ?? '—'} decisions
                    </p>
                  </div>
                  <div className="bg-background border border-border rounded p-2 text-center">
                    <p className="text-[9px] text-muted-foreground uppercase tracking-wider font-semibold">Reroute Rate</p>
                    <p className="text-lg font-mono font-bold text-amber-400 mt-0.5">
                      {rerouting.pct_zones_rerouted?.toFixed(1) ?? '—'}%
                    </p>
                    <p className="text-[9px] text-muted-foreground font-mono">of zone-updates</p>
                  </div>
                  <div className="bg-background border border-border rounded p-2 text-center">
                    <p className="text-[9px] text-muted-foreground uppercase tracking-wider font-semibold">Contribution</p>
                    <p className="text-lg font-mono font-bold text-green-400 mt-0.5">
                      {rerouting.rerouting_contribution_pct?.toFixed(1) ?? '—'}%
                    </p>
                    <p className="text-[9px] text-muted-foreground font-mono">of time improvement</p>
                  </div>
                </div>

                {rerouting.description && (
                  <p className="text-[10px] text-slate-400 leading-relaxed font-mono border-t border-border/40 pt-3">
                    {rerouting.description}
                  </p>
                )}
              </div>
        ...
        {!results && !isRunning && (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-center py-16">
            ...
            <p className="text-xs text-slate-500 font-mono max-w-xs leading-relaxed">
              Configure parameters above and click Run Evaluation to compare greedy baseline
              against ML-guided routing.
            </p>
          </div>
        )}
      </div>
